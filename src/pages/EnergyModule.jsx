...
import { 
  Zap, Calendar, Activity, Download, 
...
const MultiDeviceSelect = ({ devices, selected, onChange, loading }) => {
  ...
  const toggleSelection = (id) => {
    ...
    if (id === 'ALL') {
      ...
      if (newSelection.length === 0) newSelection = ['ALL']; // fallback
    }
...
const EnergyModule = () => {
  ...
  // NEW: State arrays to support dynamic multi-device comparison
  const [selectedMeters, setSelectedMeters] = useState(['ALL']);
  
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [peakTimeframe, setPeakTimeframe] = useState('day'); 
  ...
  useEffect(() => {
    const initDates = async () => {
      setLoading(true);
      const { data } = await supabase.from('power_consumption').select('recorded_at').order('recorded_at', { ascending: false }).limit(1);

      if (data && data.length > 0) {
        const latestDate = new Date(data[0].recorded_at);
        const firstDate = new Date(latestDate);
        firstDate.setDate(latestDate.getDate() - 1); 
  ...
  useEffect(() => {
    ...
    const fetchTargetedData = async () => {
      ...
      const { count, error: countError } = await supabase.from('power_consumption').select('recorded_at', { count: 'exact', head: true }).gte('recorded_at', startIso).lt('recorded_at', endIso);
      ...
      for (let i = 0; i < count; i += chunkSize * maxConcurrent) {
         ...
         for (let j = 0; j < maxConcurrent; j++) {
            ...
            promises.push(
               supabase.from('power_consumption').select('recorded_at, meter_sn, hourly_kwh').gte('recorded_at', startIso).lt('recorded_at', endIso).order('recorded_at', { ascending: true }).range(from, from + chunkSize - 1)
            );
         }
  ...
  // NEW: Dynamic array of colors and specific devices for the comparison tool
  const availableDevices = useMemo(() => {
    const colors = ['#3b82f6', '#10b981', '#8b5cf6', '#ec4899', '#14b8a6', '#f59e0b', '#6366f1', '#f43f5e', '#84cc16', '#a855f7', '#0ea5e9'];
    ...
    return [{ id: 'ALL', name: 'Facility Aggregated', color: '#f97316' }, ...devices];
  }, [rawData]);
  ...
  const handleGranularityChange = (t) => {
    ...
    if (t === 'hourly') start.setDate(end.getDate() - 1);
    else if (t === 'daily') start.setDate(end.getDate() - 14); 
  ...
  const { chartData, metrics } = useMemo(() => {
    ...
    filtered.forEach(row => {
      ...
      if (granularity === 'hourly') label = date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      else if (granularity === 'daily' || granularity === 'cumulative') label = date.toLocaleDateString([], { month: 'short', day: 'numeric' });
    ...
    // Fix Double Counting for the KPI
    ...
    return { 
      ...
      metrics: { 
        totalUsage: kpiTotal, 
        formattedUsage: kpiTotal.toLocaleString('en-US', { maximumFractionDigits: 0 }) 
      } 
    };
  ...
  const projectedMonthly = useMemo(() => {
    ...
    return (dailyAverage * 30).toLocaleString('en-US', { maximumFractionDigits: 0 });
  ...
  const highestPeakDevice = useMemo(() => {
    ...
    timeframeData.forEach(row => {
      ...
      deviceTotals[row.meter_sn] += parseFloat(row.hourly_kwh || 0);
    });
  ...
  const handleExport = () => {
    ...
    const csvRows = [
      ["Recorded At", "Meter Serial", "Hourly Usage (kWh)"],
      ...rawData
        .filter(r => {
        ...
        .map(r => [r.recorded_at, r.meter_sn, r.hourly_kwh])
    ];
    const csvContent = "data:text/csv;charset=utf-8," + csvRows.map(e => e.join(",")).join("\n");
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Energy_Export_Filtered_${startDate}_to_${endDate}.csv`);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center"><Zap className="mr-2 text-orange-500" /> Energy Operation Centre</h2>
          <div className="flex items-center mt-1 space-x-2">
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">
              {loading ? 'Compiling Edge Nodes...' : 'Edge Online: Syncing Active Window'}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button disabled={loading} onClick={handleExport} className="bg-[#005f5f] disabled:bg-slate-400 text-white px-4 py-2 rounded-lg text-xs font-bold shadow-sm flex items-center transition-colors uppercase"><Download size={14} className="mr-2" /> Excel</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <DashboardCard title={selectedMeters.includes('ALL') ? "Period Usage" : "Selected Load"} mainValue={loading ? <SkeletonPulse /> : `${metrics.formattedUsage} kWh`} icon={TrendingUp} color="blue" onClick={() => !loading && handleGranularityChange('hourly')}>
        ...
        <DashboardCard title={selectedMeters.includes('ALL') ? "Monthly Projected" : "Projected Load"} mainValue={loading ? <SkeletonPulse /> : `${projectedMonthly} kWh`} icon={Calendar} color="orange" onClick={() => !loading && handleGranularityChange('monthly')}>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm relative hover:shadow-md transition-shadow cursor-pointer group">
           <div className="flex justify-between items-start">
             ...
             <div className="p-2 rounded-lg bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400"><Zap size={20} /></div>
           </div>
           
           <div className="mt-4 flex justify-between items-center">
             <select disabled={loading} value={peakTimeframe} onChange={(e) => { e.stopPropagation(); setPeakTimeframe(e.target.value); }} className="text-[10px] font-bold text-purple-600 uppercase tracking-tighter bg-purple-50 dark:bg-purple-900/20 border-none outline-none rounded p-1 cursor-pointer disabled:opacity-50">
                <option value="day">Past 24H</option><option value="week">Past Week</option><option value="month">Past Month</option>
             </select>
             <button disabled={loading} onClick={() => navigate(`/energy/devices?highlight=${highestPeakDevice}`)} className="text-[10px] font-bold text-slate-400 hover:text-purple-600 flex items-center uppercase transition-colors disabled:opacity-50"><ArrowUpRight size={10} className="mr-1" /> Floorplan</button>
           </div>
        </div>
        
        <DashboardCard title="Efficiency Status" mainValue="Monitoring" icon={Activity} color="green" onClick={() => navigate('/energy/logs')}>
           <div className="text-[10px] font-bold text-green-600 uppercase tracking-tighter mt-2">Audit Logs & Fault Detection</div>
        </DashboardCard>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm min-h-[500px] flex flex-col">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4 border-b border-slate-100 dark:border-slate-700 pb-4">
          <div className={`flex bg-slate-100 dark:bg-slate-900 p-1 rounded-lg ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
            {['hourly', 'daily', 'weekly', 'monthly', 'yearly', 'cumulative'].map(t => (
              <button key={t} onClick={() => handleGranularityChange(t)} className={`px-4 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${granularity === t ? 'bg-[#005f5f] text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
          ...
          <div className="flex items-center gap-6">
             <label className={`flex items-center gap-2 cursor-pointer group ${loading || !selectedMeters.includes('ALL') ? 'opacity-50 pointer-events-none' : ''}`}>
                <input disabled={loading || !selectedMeters.includes('ALL')} type="checkbox" checked={showProjected} onChange={() => setShowProjected(!showProjected)} className="w-4 h-4 rounded text-orange-500" />
                <span className={`text-[11px] font-bold uppercase ${showProjected ? 'text-orange-600' : 'text-slate-400'}`}>Projected</span>
             </label>
             <label className={`flex items-center gap-2 cursor-pointer group ${loading || !selectedMeters.includes('ALL') ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="w-full h-[400px] mt-2 relative">
          {loading ? (
            <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
              <RefreshCw className="animate-spin text-[#005f5f] mb-4" size={32} />
              <span className="text-xs font-bold text-[#005f5f] uppercase tracking-widest animate-pulse">Aggregating Telemetry...</span>
              <div className="flex items-end gap-2 mt-6 h-16">
                 <div className="w-3 bg-[#005f5f]/20 rounded animate-pulse h-6" style={{ animationDelay: '0ms' }}></div>
                 <div className="w-3 bg-[#005f5f]/40 rounded animate-pulse h-10" style={{ animationDelay: '150ms' }}></div>
                 <div className="w-3 bg-[#005f5f]/60 rounded animate-pulse h-16" style={{ animationDelay: '300ms' }}></div>
                 <div className="w-3 bg-[#005f5f]/80 rounded animate-pulse h-8" style={{ animationDelay: '450ms' }}></div>
                 <div className="w-3 bg-[#005f5f] rounded animate-pulse h-12" style={{ animationDelay: '600ms' }}></div>
              </div>
            </div>
          ) : chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData}>
                ...
                {showProjected && selectedMeters.includes('ALL') && <RechartsLine type="monotone" dataKey="projected" name="Projected Load (All)" stroke="#f97316" strokeDasharray="4 4" dot={false} strokeWidth={2} />}
